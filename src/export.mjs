 ...
 */
/* The colours that live OUTSIDE the twelve numbered steps and outside the per-family ink.
 *
 * Declared once because they were previously hand-written into the Figma, Swift and Kotlin
 * exporters separately — so adding --ink-max and --void broke all three at once, and only the
 * DTCG parity assertion noticed. Three hand-maintained copies of one list is the same defect
 * this package keeps finding in other people's code.
 *
 * Values come from the palette where the solver put them, and are restated where it did not:
 * css.mjs derives ink-max and void from the hue rather than storing them on the scale. */
const extraColours = (palette) => {
  const h = palette.light.neutral[1].h;
  return [
    ['surface', '--surface — a raised surface. The only one with no tint.',
      palette.light.neutral.surface, palette.dark.neutral.surface],
    ['ink-max', '--ink-max — maximum ink, not step 12. For a QR code or a barcode, where all '
      + 'the available contrast is wanted rather than enough for text.',
      { L: 0.16, C: 0, h }, { L: 1, C: 0, h }],
    ['void', '--void — below the page, which no numbered step is. A video matte, a letterbox, '
      + 'an editor gutter: a surface that must read as absence rather than as another panel.',
      { L: 0.90, C: 0.007, h }, { L: 0.10, C: 0.007, h }],
  ];
};
...
export function toFigmaVariables(palette, opts = {}) {
  ...
  for (const [name, desc, light, dark] of extraColours(palette)) {
    add(`color/${name}`, 'COLOR', desc, {
      cssVar: `--${name}`,
      light: figmaColour(light),
      dark: figmaColour(dark),
    });
  }
...
export function toSwift(palette, opts = {}) {
  ...
  lines.push('\n    // MARK: outside the numbered steps');
  for (const [name, desc, light, dark] of extraColours(palette)) {
    const ident = name.replace(/-(\w)/g, (_, c) => c.toUpperCase());
    lines.push(`    /// ${desc}`);
    lines.push(
      `    public static let ${ident} = dynamic(RGB(${rgbTriplet(light)}), `
      + `RGB(${rgbTriplet(dark)}))`,
    );
  }
...
export function toKotlin(palette, opts = {}) {
  ...
  for (const [name] of extraColours(palette)) names.push([name, 'neutral', name]);
  ...
  const at = (mode, family, step) => {
    ...
    if (step === 'ink-max') return mode === 'light' ? { L: 0.16, C: 0, h } : { L: 1, C: 0, h };
    if (step === 'void') return mode === 'light' ? { L: 0.90, C: 0.007, h } : { L: 0.10, C: 0.007, h };
